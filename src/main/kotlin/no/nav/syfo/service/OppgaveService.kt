...
class OppgaveService(
    ...
    suspend fun upsertOppgave(
        ...
        return if (
            papirSmRegistering.oppgaveId == null ||
                (papirSmRegistering.oppgaveId.toIntOrNull() == null)
        ) {
            val opprettOppgave =
                OpprettOppgave(
            ...
            val opprettetOppgave =
                oppgaveClient.opprettOppgave(opprettOppgave, papirSmRegistering.sykmeldingId)
    ...
    suspend fun patchManuellOppgave(oppgaveId: Int, msgId: String): Oppgave {
        ...
        if (oppgave.status == "FERDIGSTILT") {
            log.warn(
                "Oppgave med id $oppgaveId er allerede ferdigstilt. Oppretter ny oppgave for msgId $msgId"
            )
