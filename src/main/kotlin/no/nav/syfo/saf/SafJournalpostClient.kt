...
class SafJournalpostClient(
    ...
    suspend fun getJournalpostMetadata(
        ...
        val getJournalpostRequest =
            GetJournalpostRequest(
                query = graphQlQuery,
                variables = GetJournalpostVariables(journalpostId)
            )
        return try {
            httpClient
                .post(basePath) {
            ...
            log.error(
                "Noe gikk galt ved henting av journalpostmetadata for journalpostid $journalpostId mot SAF: ${e.message}"
            )
            null
        }
    }
