...
class ReceivedSykmeldingControllerTest {
    ...
    fun beforeEachTest() {
        ...
        coEvery { oppgaveClient.opprettOppgave(any(), any()) } returns
            Oppgave(id = 1, oppgavetype = "JFR", aktivDato = LocalDate.now(), prioritet = "")
        coEvery { oppgaveClient.oppdaterOppgave(any(), any()) } returns
            Oppgave(id = 2, oppgavetype = "JFR", aktivDato = LocalDate.now(), prioritet = "")
    }
    ...
    fun oppretterIkkeOppgaveHvisFinnesIDatabaseFraFor() {
        ...
        runBlocking { controller.handleReceivedSykmelding(papirSmRegistrering, loggingMeta) }
    ...
    fun opprettFerdigstiltPapirsykmelding(): String {
        ...
        database.insertSendtSykmeldingHistory(
